...
static void wbgcm_mult( WBCRYPTO_wbgcm_ee_context *ctx,
...
//        h[i] = key->GBOX_TypeIII[i][correctMsg[i]][h[i]];
        unsigned char u8_front4 = key->GBOX_TypeIII[i][0][(correctMsg[i] >> 4) & 0x0f][(h[i] >> 4) & 0x0f];
        unsigned char u8_back4 = key->GBOX_TypeIII[i][1][(correctMsg[i] & 0x0f)][(h[i] & 0x0f)];
        h[i] = u8_front4 << 4 | u8_back4;
    }
...
//            out[i] = key->GBOX_TypeIII[i][input_change[i]][ectr[i]];
            unsigned char u8_front4 = key->GBOX_TypeIII[i][0][(input_change[i] >> 4) & 0x0f][(ectr[i] >> 4) & 0x0f];
            unsigned char u8_back4 = key->GBOX_TypeIII[i][1][(input_change[i] & 0x0f)][(ectr[i] & 0x0f)];
            out[i] = u8_front4 << 4 | u8_back4;
...
//            tag[i] = key->GBOX_TypeIII[i][buff_change[i]][tag[i]];
            unsigned char u8_front4 = key->GBOX_TypeIII[i][0][(buff_change[i] >> 4) & 0x0f][(tag[i] >> 4) & 0x0f];
            unsigned char u8_back4 = key->GBOX_TypeIII[i][1][(buff_change[i] & 0x0f)][(tag[i] & 0x0f)];
            tag[i] = u8_front4 << 4 | u8_back4;
        }
    }
    return 0;
}
