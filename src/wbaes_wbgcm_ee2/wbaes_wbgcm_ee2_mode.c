...
static void wbgcm_mult( WBCRYPTO_wbgcm_ee2_context *ctx,
...
WBCRYPTO_wbgcm_ee2_context *WBCRYPTO_wbaes_wbgcm_ee2_init(WBCRYPTO_wbaes_wbgcm_ee2_context *key)
{
    ...
    ctx=malloc(sizeof(struct wbgcm_ee2_context));
    if(ctx==NULL){
        return NULL;
    }
    memset( ctx, 0, sizeof(WBCRYPTO_wbgcm_ee2_context) );  // zero caller-provided GCM context
    memset( h, 0, 16 );                     // initialize the block to encrypt

    ctx->key=key;
    ctx->block=(block128_f) aux_WBCRYPTO_wbaes_wbgcm_ee2_encrypt;
...
int WBCRYPTO_wbaes_wbgcm_ee2_setiv(WBCRYPTO_wbgcm_ee2_context *ctx,
    ...
    WBCRYPTO_wbaes_wbgcm_ee2_context *key = ctx->key;
...
void WBCRYPTO_wbaes_wbgcm_ee2_free(WBCRYPTO_wbgcm_ee2_context *ctx){
    memset( ctx, 0, sizeof(WBCRYPTO_wbgcm_ee2_context) );
    if(ctx!=NULL){
        free(ctx);
        ctx=NULL;
    }
